...
local function getEnemyPivot(model)
    -- Kembalikan CFrame + posisi target walau tanpa HumanoidRootPart
    local hrp = model:FindFirstChild("HumanoidRootPart")
    if hrp and hrp:IsA("BasePart") then return hrp.CFrame, hrp.Position, hrp end
    local pp = model.PrimaryPart
    if pp and pp:IsA("BasePart") then return pp.CFrame, pp.Position, pp end
    for _, c in ipairs(model:GetChildren()) do
        if c:IsA("BasePart") then return c.CFrame, c.Position, c end
    end
    local ok, cf = pcall(function() return model:GetPivot() end)
    if ok and cf then return cf, cf.Position, nil end
    return nil, nil, nil
end
...
-- Prop yg bisa diserang: Chest & Dragon Egg (model tanpa Humanoid).
-- Dipukul fisik (teleport + Activate) sama seperti musuh.
local function findBreakables(myPos)
    local list = {}
    local scopes = {}
    local tc = workspace:FindFirstChild("TreasureChests")
    if tc then table.insert(scopes, tc) end
    local de = workspace:FindFirstChild("DragonEggs")
    if de then table.insert(scopes, de) end
    -- Model Chest*/DragonEgg* yg jadi anak langsung workspace
    for _, v in ipairs(workspace:GetChildren()) do
        if v:IsA("Model") then
            local nm = string.lower(v.Name)
            if string.find(nm, "chest", 1, true) or string.find(nm, "egg", 1, true) then
    ...
    for _, scope in ipairs(scopes) do
        local models = {}
        if scope:IsA("Model") then models = { scope }
        else
            for _, v in ipairs(scope:GetChildren()) do
                if v:IsA("Model") then table.insert(models, v) end
            end
        end
        for _, model in ipairs(models) do
            local ok, info = pcall(function()
                if model:FindFirstChildOfClass("Humanoid") then return nil end -- yg ada humanoid = mob, bukan prop
                local cf, pos, part = getEnemyPivot(model)
                if not pos then return nil end
                local dist = (pos - myPos).Magnitude
                if not Config.UnlimitedRange and dist > Config.Radius then return nil end
                return { Model = model, Humanoid = nil, Root = part, Pos = pos, Dist = dist, Kind = "prop" }
            end)
            if ok and info then
                table.insert(list, info)
...
local function findTargets(myPos)
    local mobs = findEnemies(myPos)
    local props = findBreakables(myPos)
    for _, p in ipairs(props) do table.insert(mobs, p) end
    return mobs
end
...
local function attackOnce()
    ...
    -- Auto hit TANPA visual hit (tidak ubah Size/Transparency target sama sekali)
